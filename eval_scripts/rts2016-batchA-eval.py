...
# (scenario A) with batch NIST assessor judgments, v1.01.
#
# Release History:
#
# - v1.01 (Dec 2016):
#     + Computes latency per topic. Latency is computed with respect
#       to the first tweet in each cluster. Note "All" latency
#       (aggregate over all topics) is a MICRO-average.
#
                ...
                for tweetid in run_dt[topic][day]:
                    ...
                    if tweetid in clusters_day_dt[topic][day]:
                        ...
                        if clusterid not in exist_clusterids:
                            ...
                            delay = (
                                int(float(run_epoch_dt[topic][tweetid])) - int(tweet2epoch_dt[first_tweetid_in_cluster]))
                            delay = max(0, delay)
                    gains.append(gain)
                    if gain > 0:
